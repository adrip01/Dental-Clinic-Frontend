...
function EditAppointmentPage() {
  ...
  const getAppointment = async (token, id) => {
    setIsLoading(true);
    try {
      const data = await userService.getAppointment(token, id);
  ...
  const updateAppointment = async () => {
    setIsLoading(true);
    try {
      const data = await userService.updateAppointment(token, formValues, id);
      console.log(data);
      setSuccess(true);
            ...
            <Grid>
              <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <Button
                  ...
                  sx={{ mt: 3 }}
                  onClick={() => {
                    updateAppointment(token, formValues, id);
                  }}
                >
                  Save changes
                </Button>
